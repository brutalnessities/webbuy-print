...
// Render function
export function compile(data: EmailPrintData): string {
...
export function compileEmail(data) {
  // Load partials
  ...
  const productTemplate = Handlebars.compile(body);

  // Render parts
  const renderedHeader = Handlebars.compile(header)(data);
  const renderedFooter = Handlebars.compile(footer)(data);
  const renderedProducts = data.products.map(p => productTemplate(p)).join("");
