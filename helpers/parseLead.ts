...
function itemData(data: any): ItemData | null {
  const item = data?.item;
  if (!item?.vin) return null;
  ...
  return {
    details: [
      ...
      { title: "Mileage", value: formatNumber(mileage) },
      { title: "VIN", value: vin },
...
function tradeData(data: any): TradeData | null {
  const trade = data?.trade;
  const tradePricing = data?.pricing?.Request?.TradeIn;
  if (!trade?.vin) return null;
  ...
  let adjustmentsRow: RowItem[] = [];
  if (adjustments?.length) {
    for (const adjustment of adjustments) {
      adjustmentsRow.push({ title: adjustment.title, value: formatCurrency(adjustment.price), calculate: adjustment.calculate });
    }
  }
  ...
  if (fees?.length) {
    for (const fee of fees) {
      feesRow.push({ title: fee.title, value: formatCurrency(fee.price), calculate: fee.calculate });
    }
  }

  return {
    valuation: [
      { title: "Trade-In Equity", value: formatCurrency(tradePricing?.Equity) },
      { title: "Book Value", value: formatCurrency(tradePricing?.Offer) },
    ],
    details: [
      ...
      { title: "Mileage", value: formatNumber(mileage) },
      { title: "VIN", value: vin },
      { title: "Lien", value: formatCurrency(lien) },
...
function pricingData(data: any): PricingData {
  ...
  if (dealType === DealType.LEASE) {
    ...
    breakdown = [
      { title: "Cash Down", value: formatCurrency(pricing?.FullDetails?.DownpaymentFromCash) },
      { title: "Gross Capitalized Cost", value: formatCurrency(pricing?.FullDetails?.AdjustedSellingPrice) },
      { title: "Total Cap. Cost Reduction", value: formatCurrency(pricing?.FullDetails?.Downpayment) },
      { title: "Adjusted Cap. Cost", value: formatCurrency(pricing?.AmountFinanced) },
  ...
  // create html for finance/cash deal type
    ...
    breakdown = [
      { title: "Total Down Payment", value: formatCurrency(pricing?.FullDetails?.Downpayment) },
      { title: "Remaining Balance", value: formatCurrency(data?.finance?.pricing?.Request?.RetailPart?.RemainingBalance) },
    ].filter((item) => item.value);
  }
  ...
  const summary = [
    { title: "Dealer Price", value: formatCurrency(dealerPrice) },
    { title: "Your Price", value: formatCurrency(discountPrice, true) },
  ].filter((item) => item.value);
